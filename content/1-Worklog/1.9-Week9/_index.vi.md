...
* Kiểm thử thành công cơ chế Alarm, SNS và Email.
...
| --- | --- | --- | --- | --- |
| 2 | - Bổ sung CloudWatch Alarm cảnh báo lỗi Lambda: <br>&emsp; + Trao đổi với nhóm về kế hoạch công việc trong ngày trước khi bắt đầu. <br>&emsp; + Cấu hình CloudWatch Alarm giám sát metric `Errors` của Lambda Collector và Analyzer. <br>&emsp; + Gắn action gửi thông báo qua SNS khi Alarm kích hoạt. <br>&emsp; + Triển khai bằng Terraform (`aws_cloudwatch_metric_alarm`). <br>&emsp; + Cuối ngày tổng hợp và chia sẻ kết quả với nhóm. | 06/07/2026 | 06/07/2026 | - Terraform aws_cloudwatch_metric_alarm: <br> https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/cloudwatch_metric_alarm <br> - Using CloudWatch Alarms: <br> https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/AlarmThatSendsEmail.html |
| 3 | - Bổ sung Alarm cho DLQ và kiểm thử Alarm: <br>&emsp; + Trao đổi với nhóm về kế hoạch công việc trong ngày trước khi bắt đầu. <br>&emsp; + Cấu hình Alarm giám sát metric `ApproximateNumberOfMessagesVisible` của Dead Letter Queue. <br>&emsp; + Gửi message lỗi để kích hoạt Alarm, kiểm thử thành công cơ chế Alarm, SNS và Email. <br>&emsp; + Xác nhận nhận được email cảnh báo sự cố với đầy đủ thông tin. <br>&emsp; + Cuối ngày tổng hợp và chia sẻ kết quả với nhóm. | 07/07/2026 | 07/07/2026 | - Amazon SQS CloudWatch Metrics: <br> https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-monitoring-using-cloudwatch.html |
| 4 | - Cập nhật sơ đồ kiến trúc (chuyển hướng dashboard): <br>&emsp; + Trao đổi với nhóm về kế hoạch công việc trong ngày trước khi bắt đầu. <br>&emsp; + Loại bỏ Athena + QuickSight khỏi sơ đồ. <br>&emsp; + Bổ sung hướng web dashboard tự xây: Lambda-API, API Gateway, CloudFront, S3 (Web Hosting). <br>&emsp; + Vẽ lại luồng: User tới CloudFront/S3 (tải giao diện) và User tới API Gateway tới Lambda-API tới S3 (lấy dữ liệu). <br>&emsp; + Cuối ngày tổng hợp và chia sẻ kết quả với nhóm. | 08/07/2026 | 08/07/2026 | - AWS Architecture Icons & công cụ vẽ sơ đồ (draw.io). |
| 5 | - Viết Lambda-API (backend cung cấp dữ liệu): <br>&emsp; + Trao đổi với nhóm về kế hoạch công việc trong ngày trước khi bắt đầu. <br>&emsp; + Viết hàm Lambda-API (Python) đọc và tổng hợp dữ liệu chi phí từ S3 (tổng chi phí, theo ngày kèm trạng thái, top dịch vụ). <br>&emsp; + Trả về JSON kèm CORS header để web frontend gọi được. <br>&emsp; + Áp dụng IAM Role riêng theo Least Privilege (chỉ đọc S3 và ghi log). <br>&emsp; + Cuối ngày tổng hợp và chia sẻ kết quả với nhóm. | 09/07/2026 | 09/07/2026 | - AWS Lambda Developer Guide (Python): <br> https://docs.aws.amazon.com/lambda/latest/dg/lambda-python.html <br> - Boto3 S3 Client: <br> https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/s3.html |
| 6 | - Tạo API Gateway và kiểm thử API: <br>&emsp; + Trao đổi với nhóm về kế hoạch công việc trong ngày trước khi bắt đầu. <br>&emsp; + Tạo HTTP API Gateway với route `GET /costs`, bật CORS. <br>&emsp; + Cấp quyền cho API Gateway gọi Lambda (`aws_lambda_permission`). <br>&emsp; + Kiểm thử API thành công: gọi endpoint trên trình duyệt và nhận về dữ liệu JSON đúng cấu trúc. <br>&emsp; + Cuối ngày tổng hợp và chia sẻ tiến độ với nhóm. | 10/07/2026 | 10/07/2026 | - Amazon API Gateway (HTTP API): <br> https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api.html <br> - Terraform aws_apigatewayv2_api: <br> https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/apigatewayv2_api |
